...
class NeuroSymbolicECG(nn.Module):
    ...
    def forward(self, x, s):
        return self.fc(torch.cat([self.cnn(x), s], dim=1))
...
for epoch in range(20):
    ...
    if val_auc > best_val_auc:
        ...
        torch.save(model.state_dict(), "hcm_neurosymbolic_v3.pt")
        no_improve = 0
    else:
        no_improve += 1
...
model.load_state_dict(torch.load("hcm_neurosymbolic_v3.pt"))
model.eval()
...
print("\n--- V3 Results (focal loss, patient-split, best checkpoint) ---")
...
print("Model saved to hcm_neurosymbolic_v3.pt")
